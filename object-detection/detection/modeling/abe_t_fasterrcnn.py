"""
NOTE: this is a direct copy of the detectron2 FasterRCNN implementation but with our AbeT method added
"""
import logging
...
class CosineHead(nn.Module):
    def __init__(self, cls_score, in_features, num_classes):
        super(CosineHead, self).__init__()

        # self.h = nn.Linear(in_features, num_classes, bias= False).to('cuda:0')
        self.h = cls_score
        self.init_weights()
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.h = CosineHead(self.cls_score, input_size, num_classes + 1)
        ...
        nn.init.normal_(self.h.h.weight, std=0.01)
        nn.init.normal_(self.bbox_pred.weight, std=0.001)
        for l in [self.h.h, self.bbox_pred]:
    ...
    def forward(self, x):
        ...
        temperature = self.temperature(x) + 1e-12
        logit_numerators = self.h(x)
        scores = logit_numerators / temperature
