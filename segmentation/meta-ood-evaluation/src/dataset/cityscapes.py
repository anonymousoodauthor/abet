...
class Cityscapes(Dataset):
    ...
    def __init__(
        self,
        root: str = "/your/path/to/Abet/datasets/cityscapes",
        split: str = "val",
        mode: str = "gtFine",
        target_type: str = "semantic_train_id",
        ...
        extra_img_path = "leftImg8bit_trainvaltest"
        extra_tar_path = "gtFine_trainvaltest" if self.mode == "gtFine" else "gtCoarse"
        self.transform = transform
        self.images_dir = os.path.join(self.root, extra_img_path, "leftImg8bit", self.split)
        self.targets_dir = os.path.join(self.root, extra_tar_path, self.mode, self.split)
        self.predictions_dir = (
            os.path.join(predictions_root, self.split) if predictions_root is not None else ""
        )
        self.images = []
        self.targets = []
        self.predictions = []
        ...
        for city in os.listdir(self.images_dir):
            ...
            for file_name in os.listdir(img_dir):
                ...
                self.predictions.append(
                    os.path.join(pred_dir, file_name.replace("_leftImg8bit", ""))
                )
    ...
    def _get_target_suffix(mode: str, target_type: str) -> str:
        if target_type == "instance":
            ...
            print(
                "'%s' is not a valid target type, choose from:\n" % target_type
