...
class LostAndFound(Dataset):
    ...
    LostAndFoundClass = namedtuple(
        "LostAndFoundClass", ["name", "id", "train_id", "category_name", "category_id", "color"]
    )
    ...
    def __init__(
        self,
        split="test",
        root="/your/path/to/Abet/datasets/lost_and_found",
        transform=None,
    ):
        """Load all filenames."""
        ...
        for root, _, filenames in os.walk(os.path.join(root, "leftImg8bit", self.split)):
            for filename in filenames:
                if os.path.splitext(filename)[1] == ".png":
                    ...
                    self.targets.append(
                        os.path.join(
                            target_root, city, filename_base + "_gtCoarse_labelTrainIds.png"
                        )
                    )
                    self.annotations.append(
                        os.path.join(target_root, city, filename_base + "_gtCoarse_labelIds.png")
                    )
