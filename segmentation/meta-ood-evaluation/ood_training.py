...
from src.imageaugmentations import Compose, Normalize, RandomCrop, RandomHorizontalFlip, ToTensor
...
def encode_target(target, pareto_alpha, num_classes, ignore_train_ind, ood_ind=254):
    ...
    enc[(npy == num_classes)] = np.full(
        num_classes, pareto_alpha / num_classes
    )  # set all hot encoded vector
...
def training_routine(config):
    ...
    for epoch in range(start_epoch, start_epoch + epochs):
        ...
        trainloader = config.dataset(
            "train", transform, roots.cs_root, roots.coco_root, params.ood_subsampling_factor
        )
        ...
        for x, target in dataloader:
            ...
            y = encode_target(
                ...
                num_classes=dataset.num_classes,
                ignore_train_ind=dataset.void_ind,
