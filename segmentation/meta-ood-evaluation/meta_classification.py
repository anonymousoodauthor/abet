...
from src.model_utils import inference, probs_gt_load
...
try:
    ...
    print(
        "Examine the README at AbeT/meta-ood-evaluation/REAME.md or the original at https://github.com/robin-chan/meta-ood"
    )
    exit()
...
class compute_metrics(object):
    ...
    def __init__(self, params, roots, dataset, num_imgs=None, metaseg_dir=None, num_cores=1):
        ...
        if self.epoch == 0:
            ...
            self.load_dir = os.path.join(
                roots.io_root, "probs/epoch_" + str(self.epoch) + "_alpha_" + str(self.alpha)
            )
            self.save_subdir = (
                "epoch_" + str(self.epoch) + "_alpha_" + str(self.alpha) + "_t" + str(self.thresh)
            )
    ...
    def compute_metrics_pred_i(self, i):
        ...
        metrics, components = compute_metrics_components(
            probs=probs, gt_train=gt_train, ood_mask=ent_mask, ood_index=self.dataset.train_id_out
        )
    ...
    def compute_metrics_gt_i(self, i):
        ...
        ent_mask = entropy_segments_mask(probs, self.thresh)
        metrics, components = compute_metrics_mask(
            probs=probs,
            mask=ent_mask,
            gt_train=gt_train,
            gt_label=gt_label,
        ...
        components_dump(
            components, i, metaseg_root=self.metaseg_dir, subdir=self.save_subdir + "_gt"
        )
        print("image", i, "processed in {}s\r".format(round(time.time() - start_i)))
...
class meta_classification(object):
    ...
    def classifier_fit_and_predict(self):
        ...
        metrics, start = concatenate_metrics(
            metaseg_root=self.metaseg_dir, subdir=self.load_subdir, num_imgs=self.num_imgs
        )
        Xa, _, _, y0a, X_names, class_names = metrics_to_dataset(
            metrics, self.dataset.num_eval_classes
        )
    ...
    def remove(self):
        ...
        fp_after = np.sum([metrics["kick"] == 1]) - np.sum(
            np.array(metrics["iou0"])[metrics["kick"] == 1]
        )
...
def main(args):
    ...
    datloader = config.dataset(
        root=config.roots.eval_dataset_root, split="test", transform=transform
    )
    start = time.time()
    ...
    if args["segment_search"]:
        ...
        compute_metrics(
            config.params, config.roots, datloader, num_cores=cpu_count() - 2
